...
StudentAfter1Session::StudentAfter1Session(int studentNum, string &name, int curs1, int group, const int *marks)
        : Student(studentNum, name, curs1, group) {
    for (int i = 0; i < 4; ++i) {
...
ostream &operator<<(ostream &out, const StudentAfter1Session &classEx) {
    ...
    cout << student;
    out << "Marks after 1st session:  " << classEx.marks[0] << " " << classEx.marks[1] << " " << classEx.marks[2] << " "
    << classEx.marks[3] << endl;
    return out;
}
